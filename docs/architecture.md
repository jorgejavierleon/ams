...
- **Dates and times in mobile payloads are naive wall-clock strings** — `Y-m-d` and `H:i:s`, no offset (`TodayResource`). The app parses them as strings and refuses anything carrying an offset, because a shift window or a punch silently re-read in the device's timezone is a different legal fact under Resolución 38 art. 8 with nothing on screen to say it moved. `MarkResource::datetime` predates this and still emits `toIso8601String()`; do not copy it into new resources.
- **A missing geofence never blocks a punch.** `Premise.geofence_radius_meters` is nullable and so are `lat`/`lng`; `shift.geofence` on `GET /api/v1/me/today` is null when there are no coordinates and carries a null `radius_meters` when no radius is set (`App\Support\Geofence`). All three are legitimate configurations, not errors — refusing to record a punch an employee actually made is worse than recording a suspect one. The client's distance check is advisory, drawn on a card; the authoritative decision is the server's at punch time and is a separate concern from this payload.
