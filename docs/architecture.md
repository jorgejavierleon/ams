...
- **List envelopes are bare arrays by default, not `{data: [...]}`.** `AppServiceProvider` calls `JsonResource::withoutWrapping()` globally, so `SomeResource::collection(...)` returned directly serializes as a bare JSON array (`WorkdaysController`, `PendingMarkModificationsController`) — the one exception is when `->additional([...])` is chained (`LeavesController::index`), which forces a `data` wrap as a side effect. A client that expects the wrapped shape needs it forced explicitly: `response()->json(['data' => SomeResource::collection(...)])` (`DocumentsController::index`, KOL-86). Check what the mobile client actually parses before assuming either shape.
...
### Payroll export dataset (KOL-49)

`App\Services\Overtime\OvertimeExportDataset::forPeriod(CarbonInterface $start, CarbonInterface $end, array $userIds)` is the one read model the payroll export, and everything built on top of it (KOL-12, KOL-13, KOL-24), may read (PRD §7.7). It returns a `Collection<OvertimeExportLine>`, a `final readonly` DTO carrying RUT, date, hours, day type, pacto reference, approver and approval timestamp — no other constructor for that shape exists.

- **Structural, not conventional, exclusion.** Every line comes from `OvertimeAuthorization::scopeExportable()` (already `approved` + payment compensation) or from an expired `OvertimeRestDayBalance` remainder. There is no third path, so a pending, revoked, or still-spendable rest-day-compensated record cannot reach a line by construction.
- **Two sources are unioned, per KOL-47's note on this task.** The payment-compensated authorizations in range, plus expired-unconsumed rest-day balances whose `expiry_date` falls in range — the second source is what keeps a lapsed rest-day balance from silently going unpaid.
- **An expiry-sourced line is dated by `expiry_date`, not the original workday** — Art. 32 §4 makes the expired remainder payable "dentro de la remuneración del respectivo periodo", i.e. the period the expiry falls in, not the period the overtime was worked. Its `dayType`, however, is still resolved from the *original* workday date (via the balance's source authorization), because that is what KOL-12 needs to classify the hour into its legal pay bucket. The same employee can therefore carry two lines, dated differently, for what was ultimately one worked day.
- **Day type (`App\Enums\OvertimeDayType`: weekday/sunday/holiday) is derived, never stored**, from `Holiday` data and the same Sunday reasoning `SundaysReportService` uses for the DT report — one `Holiday::whereIn('date', …)` query covering every date the call actually needs (period dates plus any out-of-period original workday dates from expiry-sourced lines).
- **Query count does not scale with the employee count**: two bounded queries (authorizations, expired balances) plus their eager loads and one holiday lookup, regardless of how many `$userIds` are passed.

### Pay-bucket classification (KOL-12)

`App\Services\Overtime\OvertimePayBucketClassifier::forPeriod(CarbonInterface $start, CarbonInterface $end, array $userIds)` sorts a period's overtime into `Collection<int, OvertimePayBucketBreakdown>`, keyed by user id — one entry per requested employee, even with no data.

- **There is no statutory 100% overtime rate.** Código del Trabajo art. 32 sets one uniform 50% recargo on every overtime hour. The "HHEE 100%" column clients and competing payroll tools ask for is payroll shorthand for routing Sunday/holiday overtime to its own concept code, not a second legal percentage — art. 38's weekly-rest rules are the actual reason that day matters, not a different pay rate. `App\Enums\OvertimePayBucket` therefore buckets by day type (`ordinary_day` / `sunday_or_holiday`), not by a fictitious rate; a future export template (KOL-25/26) maps each bucket to whatever haber code the client configures in Nubox or elsewhere.
- **Four buckets, not two, because every calculated hour needs somewhere to go.** `OvertimePayBucketBreakdown` carries `ordinaryDayHours` / `sundayOrHolidayHours` (payable now, sourced from `OvertimeExportDataset`), `compensatedInRestDaysHours` (authorised but paid in time off under KOL-47, not money, and not yet lapsed unconsumed), and `unauthorizedHours` (worked but never decided on, or decided against — KOL-46). Without the third bucket, an active rest-day compensation would vanish from the reconciliation: it is authorised, so it is not unauthorised, but it is not in `OvertimeExportDataset` either, since that scope structurally excludes non-payment compensation.
- **The unauthorised and rest-day buckets read `Workday` directly**, not `OvertimeExportDataset` — that dataset only ever answers "what is payable in money," so a day nobody opened a decision for (`Workday::overtimeAuthorization` is null) still counts its full `calculated_overtime` as unauthorised via `Workday::unauthorizedOvertime()`, which is already null-safe for that case.
- **Reconciliation holds per period for the workdays in that period**, not across the expiry-driven cross-period inflow `OvertimeExportDataset` can add: an expired rest-day balance's payable line is dated by its `expiry_date`, which can fall in a different period than the workday that earned it (see above). That inflow is an intentional, documented addition to whichever period the expiry lands in — it is not drawn from, and does not need to reconcile against, the `calculated_overtime` of the period being classified.
- **Deliberately does not read `workdays.extra_time`.** That column is the legacy Resolución 38 figure (worked span minus scheduled duration); this classifier, like `OvertimeExportDataset`, is built on `calculated_overtime` (OHC, KOL-38's shift-excess figure) throughout, per KOL-12's own m-2 rescoping note.
