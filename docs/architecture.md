...
- **The engine has no word for "approved".** `workdays.overtime_state` casts to `App\Enums\OvertimeCalculationState`, whose only cases are `not_applicable` and `pending_review` (PRD §7.2: the calculation *"can reach pending review at most"*). Do not add an approved/payable case to it. The `pending | approved | objected` state machine belongs to the authorisation record of PRD §8 — a separate row the engine never writes — and keeping the two vocabularies apart is what makes "no hour is payable without a human decision" structural rather than procedural.
