...
- **The punch is server-authoritative.** `POST /api/v1/marks` rejects a client `datetime` outright (a time the device chooses is a time it can falsify, Res. 38 art. 11) and stamps the mark itself in the employee's timezone. It evaluates the geofence itself too, by haversine against the premise snapshotted onto the mark (`Geofence::verdictFor()`), and stores the verdict on `marks.geo_status`; the client's own `geo_status` travels on the request but never decides what is recorded.
...
- **Geolocation is never part of the mark's checksum.** `MarkObserver` stamps the legal snapshot and SHA-256 over who punched, which way and when; coordinates, accuracy and the geofence verdict are attached afterwards. Folding them in would make a mark's integrity depend on a phone's GPS.
