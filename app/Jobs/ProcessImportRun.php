...
class ProcessImportRun implements ShouldQueue
{
    ...
    public function handle(
        ...
        CurrentOrganization::runAs(
            ...
            fn () => $this->commit($importRun, $schema, $evaluateImportRow, $readImportFileRows, $buildColumnMappings),
        );
    }
    ...
    private function commit(
        ...
        foreach (array_chunk($remainingRows, $chunkSize, true) as $chunk) {
            DB::transaction(function () use ($chunk, $importRun, $schema, $evaluateImportRow, $columnMappings, &$counts, &$committedThrough): void {
                foreach ($chunk as $index => $rawRow) {
                    ...
                    $this->applyRow($schema, $result, $importRun, $counts);
                }
     ...
     */
    private function applyRow(EmployeeImportSchema $schema, ImportRow $result, ImportRun $importRun, array &$counts): void
    {
